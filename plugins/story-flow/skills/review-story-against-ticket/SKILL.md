...
- Report findings with specific examples of what's missing, incomplete, or misaligned. List every finding first, most consequential first, and stop there — that list is the agenda, not the review.
- Then work the agenda one finding per message: what the story says, what the source says, what you propose — and wait for the developer's reply before raising the next. Apply a finding to the story markdown once they confirm it, drop it without re-arguing if they reject it, and move on if they defer. Never batch the discussion into one message, into a plan, or into a single request to approve everything; if the developer asks for all of them applied at once, do that and say so.
